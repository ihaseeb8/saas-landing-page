          ...
          {pricingTiers.map((tier: typeof pricingTiers[0]) => (
            <div key={tier.title} className={twMerge("card", tier.inverse && "bg-black border-black text-white")}>
              <div className="flex justify-between">
                ...
                {tier.popular && 
                  <div className="inline-flex text-sm px-4 py-1.5 rounded-xl border border-white/20">
                    ...
                    className="bg-[linear-gradient(to_right,#DD7DDF,#E1CD86,#BBCB92,#71C2EF,#3BFFFF,#E1CD86,#BBCB92,#71C2EF,#3BFFFF,#DD7DDF)] [background-size:200%] text-transparent bg-clip-text font-medium">Popular</motion.span>
                  </div>
                }
              ...
              <span className="text-4xl font-bold tracking-tighter leading-none">${tier.monthlyPrice}</span>
                <span className={twMerge("tracking-tight font-bold text-black/50", tier.inverse && "text-white/50")}>/month</span>
              </div>
